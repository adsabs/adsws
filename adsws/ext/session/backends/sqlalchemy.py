...
"""
    adsws.ext.session.backends.sqlalchemy
    ---------------------------------------

    Configuration:

    - SESSION_BACKEND_SQLALCHEMY = 'adsws.ext.sqlalchemy:db'
    - SESSION_BACKEND_SQLALCHEMY_MODEL = 'adsws.ext.session.model:Session'
...
class Storage(SessionStorage):
    ...
    def model(self):
        ...
        return import_string(current_app.config.get(
            ...
            'adsws.ext.session.model:Session'))()
