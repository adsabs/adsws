...
class SessionInterface(FlaskSessionInterface):
    ...
    def has_secure_url(self):
        return current_app.config.get('SITE_SECURE_URL', '').\
            startswith("https://")
    ...
    def session_class(self):
        session_class_string = current_app.config.get(
            'SESSION_CLASS', 'adsws.ext.session.legacy_session:Session')
    ...
    def backend(self):
        storage_string = current_app.config.get(
            'SESSION_BACKEND', 'adsws.ext.session.backends.sqlalchemy:Storage')
