...
@blueprint.route('/login/openid', methods=['GET', 'POST'])
@oid.loginhandler
def login_openid():
    ...
    return render_template('blueprint/login_openid.html', form=form, error=oid.fetch_error())
...
def create_or_login(resp):
    ...
    return redirect(url_for('blueprint.create_profile', next=oid.get_next_url(),
            name=resp.fullname or resp.nickname, email=resp.email,
            openid=resp.identity_url))


@blueprint.route('/create_profile', methods=['GET', 'POST'])
def create_profile():
    if current_user.is_authenticated():
        return redirect(url_for('user.index'))
    ...
    return render_template('blueprint/create_profile.html', form=form)
...
def search():
    ...
    return render_template('blueprint/search.html', pagination=pagination, keywords=keywords)
...
def login():
    ...
    return render_template('blueprint/login.html', form=form)
...
def reauth():
    ...
    return render_template('blueprint/reauth.html', form=form)
...
def signup():
    ...
    return render_template('blueprint/signup.html', form=form)
...
def change_password():
    ...
    return render_template("blueprint/change_password.html", form=form)
...
def reset_password():
    ...
    if form.validate_on_submit():
        ...
        if user:
            ...
            url = url_for('blueprint.change_password', email=user.email, activation_key=user.activation_key, _external=True)
            ...
            return render_template('blueprint/reset_password.html', form=form)
        else:
...
def help():
    return render_template('blueprint/footers/help.html', active="help")
